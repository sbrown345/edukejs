...
/// <reference path="../../build/headers/polymer.h.ts" />
...
//// CVARS
var /*int32_t */      pr_lighting = 1;
var /*int32_t */      pr_normalmapping = 1;
var /*int32_t */      pr_specularmapping = 1;
var /*int32_t */      pr_shadows = 1;
var /*int32_t */      pr_shadowcount = 5;
var /*int32_t */      pr_shadowdetail = 4;
var /*int32_t */      pr_shadowfiltering = 1;
var /*int32_t */      pr_maxlightpasses = 10;
var /*int32_t */      pr_maxlightpriority = PR_MAXLIGHTPRIORITY;
var /*int32_t */      pr_fov = 426;           // appears to be the classic setting.
var /*double  */      pr_customaspect = 0.0;
var /*int32_t */      pr_billboardingmode = 1;
var /*int32_t */      pr_verbosity = 1;       // 0: silent, 1: errors and one-times, 2: multiple-times, 3: flood
var /*int32_t */      pr_wireframe = 0;
var /*int32_t */      pr_vbos = 2;
var /*int32_t */      pr_gpusmoothing = 1;
var /*int32_t */      pr_overrideparallax = 0;
var /*float   */      pr_parallaxscale = 0.1;
var /*float   */      pr_parallaxbias = 0.0;
var /*int32_t */      pr_overridespecular = 0;
var /*float   */      pr_specularpower = 15.0;
var /*float   */      pr_specularfactor = 1.0;
var /*int32_t */      pr_highpalookups = 1;
var /*int32_t */      pr_artmapping = 1;
var /*int32_t */      pr_overridehud = 0;
var /*float   */      pr_hudxadd = 0.0;
var /*float   */      pr_hudyadd = 0.0;
var /*float   */      pr_hudzadd = 0.0;
var /*int32_t */      pr_hudangadd = 0;
var /*int32_t */      pr_hudfov = 426;
var /*float   */      pr_overridemodelscale = 0.0;
var /*int32_t */      pr_ati_fboworkaround = 0;
var /*int32_t */      pr_ati_nodepthoffset = 0;
//#ifdef __APPLE__
...
var mapvbousage = GL_STREAM_DRAW_ARB; //GLenum          
var modelvbousage = GL_STATIC_DRAW_ARB; //GLenum          

// BUILD DATA
var prsectors = newStructArray<_prsector>(_prsector,MAXSECTORS);
var prwalls:_prwall[];// = newStructArray<_prwall>(_prwall, MAXWALLS);
var prsprites = newStructArray<_prsprite>(_prsprite, MAXSPRITES);
var mdspritematerial:_prmaterial;
var prhighpalookups:_prhighpalookup[][] = [];
for (var i = 0; i < MAXBASEPALS ; i++) {
    prhighpalookups[i] = newStructArray<_prhighpalookup>(_prhighpalookup, MAXPALOOKUPS);
}

// One U8 texture per tile
var prartmaps = new Uint32Array(MAXTILES);//GLuint          
// 256 U8U8U8 values per basepal
var prbasepalmaps = new Uint32Array(MAXBASEPALS);//GLuint          
// numshades full indirections (32*256) per lookup
var prlookups = new Uint32Array(MAXPALOOKUPS);//GLuint          
...
var skyboxdatavbo:number;//GLuint          

var artskydata = new Float32Array(16);

//// LIGHTS
//#pragma pack(push,1)
var prlights = newStructArray<_prlight>(_prlight,PR_MAXLIGHTS);
var/*int32_t         */lightcount=0;
...
var /*int32_t         */culledface=0;

// EXTERNAL FUNCTIONS
function /*int32_t             */polymer_init(): number
{
    var/*int32_t         */i:number, j:number, t = getticks();
    ...
    prsectors = [];//clearStructArray(prsectors);
    prwalls = [];//clearStructArray(prwalls); // todo: slow, maybe simplify classes

    todo("prtess = bgluNewTess();");
    //if (prtess == 0)
    //{
...
function polymer_loadboard():void
{
    var/*int32_t         */i:number;
...
function polymer_postrotatesprite():void
{
...
//function polymer_drawsprite(/*int32_t */snum:number):void
//{
//    var /*int32_t         */i:number, j:number, cs:number;
//    var _s: prsprite;
...
//    if (picanm[tspr.picnum].sf & PICANM_ANIMTYPE_MASK) tspr.picnum += animateoffs(tspr.picnum, tspr.owner+32768);
...
//    {
//        bglEnable(GL_CULL_FACE);
//        { culledface = (culledface == GL_FRONT) ? GL_BACK : GL_FRONT; bglCullFace(culledface); } /*SWITCH_CULL_DIRECTION*/;
//        todoThrow("polymer_drawmdsprite(tspr)");
//        { culledface = (culledface == GL_FRONT) ? GL_BACK : GL_FRONT; bglCullFace(culledface); } /*SWITCH_CULL_DIRECTION*/;
//        bglDisable(GL_CULL_FACE);
//        return;
//    }
...
//    {
//    case 1:
//        prsectors[tspr.sectnum].wallsproffset += 0.5;
//        if (!depth || mirrors[depth-1].plane)
//            bglPolygonOffset(-1.0, -1.0);
//        break;
//    case 2:
//        prsectors[tspr.sectnum].floorsproffset += 0.5;
//        if (!depth || mirrors[depth-1].plane)
//            bglPolygonOffset(-1.0, -1.0);
//        break;
//    }
...
function  polymer_texinvalidate():void
{
    var/*int32_t         */i:number;
...
//        Bmemcpy(localskymodelviewmatrix, curskymodelviewmatrix,  /*sizeof(GLfloat)*/ 4 * 16);
...
//        bglVertexPointer(3, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, NULL);
//        bglTexCoordPointer(2, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, (GLfloat*)(3 *  /*sizeof(GLfloat)*/ 4));
//        if (plane.indices)
//            bglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, plane.ivbo);
//    } else {
//        bglVertexPointer(3, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, plane.buffer);
//        bglTexCoordPointer(2, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, &plane.buffer[3]);
//    }
...
function polymer_freeboard():void
{
    var/*int32_t         */i:number;
    todo("polymer_freeboard");
    //i = 0;
    //while (i < MAXSECTORS)
    //{
    //    if (prsectors[i])
...
function /*static int32_t*/      polymer_initsector(/*int16_t */sectnum:number):number
{
    ...
    s.verts = new Float64Array(sec.wallnum*3);//(GLdouble *)Bcalloc(sec.wallnum, sizeof(GLdouble) * 3);
    s.floor.buffer = new Float32Array(sec.wallnum*5);//(GLfloat *)Bcalloc(sec.wallnum,  /*sizeof(GLfloat)*/ 4 * 5);
    s.floor.vertcount = sec.wallnum;
    s.ceil.buffer = new Float64Array(sec.wallnum*5);//(GLfloat *)Bcalloc(sec.wallnum,  /*sizeof(GLfloat)*/ 4 * 5);
    ...
    //    return (0);
    //}
    s.floor.vbo = gl.createBuffer();//bglGenBuffersARB(1, &s.floor.vbo);
    s.ceil.vbo = gl.createBuffer();//;bglGenBuffersARB(1, &s.ceil.vbo);
    s.floor.ivbo = gl.createBuffer();//;bglGenBuffersARB(1, &s.floor.ivbo);
    s.ceil.ivbo = gl.createBuffer();//;bglGenBuffersARB(1, &s.ceil.ivbo);
todo("bglBindBufferARB , match up params etc");
    //bglBindBufferARB(GL_ARRAY_BUFFER_ARB, s.floor.vbo);
    //bglBufferDataARB(GL_ARRAY_BUFFER_ARB, sec.wallnum *  /*sizeof(GLfloat)*/ 4 * 5, /*NULL*/0, mapvbousage);

    //bglBindBufferARB(GL_ARRAY_BUFFER_ARB, s.ceil.vbo);
    //bglBufferDataARB(GL_ARRAY_BUFFER_ARB, sec.wallnum *  /*sizeof(GLfloat)*/ 4 * 5, /*NULL*/0, mapvbousage);

    //bglBindBufferARB(GL_ARRAY_BUFFER_ARB, null);
...
function /*int32_t      */polymer_updatesector(/*int16_t */sectnum:number):number
{
    var s:_prsector;                                                      //_prsector*      
    var sec:sectortype;                                                   //sectortype      
    var wal:walltype;                                                   //walltype        
    var i=0, j=0;                                                   //int32_t         
    var ceilz=new R(0), florz=new R(0);                                           //int32_t         
    var tex=0, tey=0, heidiff=0;                                      //int32_t         
    var secangcos=0.0, secangsin=0.0, scalecoef=0.0, xpancoef=0.0, ypancoef=0.0;    //float           
    var ang=0, needfloor=0, wallinvalidate=0;                         //int32_t         
    var curstat=0, curpicnum=0, floorpicnum=0, ceilingpicnum=0;         //int16_t         
    var curxpanning=0, curypanning=0;                               //char            
    var curbuffer:Float32Array;                                              //GLfloat*        
            ...
            (sec.ceilingheinum != s.ceilingheinum))
    {
        ...
        while (i < sec.wallnum)
        {
            ...
            s.floor.buffer[(i*5)+1] = -  /*(float)*/  (florz.$) / 16.0;
            s.ceil.buffer[(i*5)+1] = -  /*(float)*/  (ceilz.$) / 16.0;
    ...
    if (picanm[floorpicnum].sf & PICANM_ANIMTYPE_MASK) floorpicnum += animateoffs(floorpicnum, sectnum);//DO_TILE_ANIM(floorpicnum, sectnum);
    ceilingpicnum = sec.ceilingpicnum;
    if (picanm[ceilingpicnum].sf & PICANM_ANIMTYPE_MASK) ceilingpicnum += animateoffs(ceilingpicnum, sectnum);//DO_TILE_ANIM(ceilingpicnum, sectnum);
    ...
    while (i < sec.wallnum)
    {
        ...
        while (j)
        {
                    ...
                    ((secangcos == 2) && (secangsin == 2)))
            {
                ...
                secangcos =  /*(float)*/  (sintable[(ang+512)&2047]) / 16383.0;
                secangsin =  /*(float)*/  (sintable[ang&2047]) / 16383.0;
            }
            ...
            if (curstat & 64)
            {
                ...
                tex = int32(xpancoef * secangsin + ypancoef * secangcos);
                tey = int32(xpancoef * secangcos - ypancoef * secangsin);
            } else {
                tex = wal.x;
                tey = -wal.y;
            }
            ...
            {
                heidiff = int32((curbuffer[(i*5)+1] - curbuffer[1]));
                // don't forget the sign, tey could be negative with concave sectors
                if (tey >= 0)
                    tey = int32(sqrt( /*(double)*/ ((tey * tey) + (heidiff * heidiff))));
                else
                    tey = -int32(sqrt( /*(double)*/ ((tey * tey) + (heidiff * heidiff))));
            }
            ...
            scalecoef = (curstat & 8) ? 8.0 : 16.0;

            if (curxpanning)
            {
                xpancoef =   /*(float)*/  (pow2long[picsiz[curpicnum] & 15]);
                xpancoef *=   /*(float)*/  (curxpanning) / (256.0 *   /*(float)*/  (tilesizx[curpicnum]));
            }
            else
                xpancoef = 0;
            ...
            if (curypanning)
            {
                ...
                ypancoef *=   /*(float)*/  (curypanning) / (256.0 *   /*(float)*/  (tilesizy[curpicnum]));
            }
            else
                ypancoef = 0;
    ...
    function attributes():number {
        ...
        {
            bglBindBufferARB(GL_ARRAY_BUFFER_ARB, s.floor.vbo);
            bglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0, sec.wallnum *  /*sizeof(GLfloat) 4 * */5, s.floor.buffer);
            bglBindBufferARB(GL_ARRAY_BUFFER_ARB, s.ceil.vbo);
            bglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0, sec.wallnum *  /*sizeof(GLfloat) 4* */  5, s.ceil.buffer);
            bglBindBufferARB(GL_ARRAY_BUFFER_ARB, null);
        }
    ...
    function finish():number {
        ...
        //        {todoThrow("create buffer ivo etc");
        //            bglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, s.floor.ivbo);
        //            bglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, s.indicescount *  /*sizeof(GLushort)*/ 2, NULL, mapvbousage);
        //            bglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, s.ceil.ivbo);
        //            bglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, s.indicescount *  /*sizeof(GLushort)*/ 2, NULL, mapvbousage);
        //            s.oldindicescount = s.indicescount;
        //        }
        //        bglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, s.floor.ivbo);
        //        bglBufferSubDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0, s.indicescount *  /*sizeof(GLushort)*/ 2, s.floor.indices);
        //        bglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, s.ceil.ivbo);
        //        bglBufferSubDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0, s.indicescount *  /*sizeof(GLushort)*/ 2, s.ceil.indices);
        //        bglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0);
        //    }
        //}
...
//        s.indicescount++;
//        s.floor.indices = (GLushort *)Brealloc(s.floor.indices, s.indicescount *  /*sizeof(GLushort)*/ 2);
//        s.ceil.indices = (GLushort *)Brealloc(s.ceil.indices, s.indicescount *  /*sizeof(GLushort)*/ 2);
//    }
...
//        s.floor.indices = (GLushort *)Bcalloc(s.indicescount,  /*sizeof(GLushort)*/ 2);
//        s.ceil.indices = (GLushort *)Bcalloc(s.indicescount,  /*sizeof(GLushort)*/ 2);
//    }
...
// WALLS
function/*int32_t      */polymer_initwall(/*int16_t*/ wallnum:number):number
{
    var w: _prwall;
    ...
    if (w.mask.buffer == NULL) {
        w.mask.buffer = new Float32Array(4*5);//(GLfloat *)Bmalloc(4 *  /*sizeof(GLfloat)*/ 4 * 5);
        w.mask.vertcount = 4;
    }
    if (w.bigportal == NULL)
        w.bigportal = new Float32Array(4 * 5);//(GLfloat *)Bmalloc(4 *  /*sizeof(GLfloat)*/ 4 * 5);
    if (w.cap == NULL)
        w.cap = new Float32Array(4 * 3);//(GLfloat *)Bmalloc(4 *  /*sizeof(GLfloat)*/ 4 * 3);

    w.wall.vbo = gl.createBuffer();//bglGenBuffersARB(1, w.wall.vbo);
    w.over.vbo = gl.createBuffer();//bglGenBuffersARB(1, w.over.vbo);
    w.mask.vbo = gl.createBuffer();//bglGenBuffersARB(1, w.mask.vbo);
    w.stuffvbo = gl.createBuffer();//bglGenBuffersARB(1, w.stuffvbo);
    todo("bglBindBufferARB params etc");
    //bglBindBufferARB(GL_ARRAY_BUFFER_ARB, w.wall.vbo);
    //bglBufferDataARB(GL_ARRAY_BUFFER_ARB, 4 *  /*sizeof(GLfloat)*/ 4 * 5, NULL, mapvbousage);
    ...
    //bglBufferDataARB(GL_ARRAY_BUFFER_ARB, 8 *  /*sizeof(GLfloat)*/ 4 * 5, NULL, mapvbousage);
...
function calc_ypancoef(/*char*/ curypanning:number, /*int16_t */curpicnum:number, /*int32_t */dopancor:number):number
{
//#ifdef NEW_MAP_FORMAT
//    if (g_loadedMapVersion >= 10)
//        return curypanning / 256.0;
//#endif
    {
        var/*float */ypancoef =   /*(float)*/  (pow2long[picsiz[curpicnum] >> 4]);
        ...
        if (dopancor)
        {
            var/*int32_t */yoffs:number;

            yoffs = ftol((ypancoef - tilesizy[curpicnum]) * (255.0 / ypancoef));
...
function polymer_updatewall(/*int16_t */wallnum:number): void
{
    var nwallnum=0, nnwallnum=0, curpicnum=0, wallpicnum=0, walloverpicnum=0, nwallpicnum=0;//int16_t         
    var curxpanning=0, curypanning=0, underwall=0, overwall=0, curpal=0;                  //char            
    var curshade=0;                                                               //int8_t          
    var wal:walltype;                                                                   //walltype        
    var sec:sectortype, nsec:sectortype;                                                            //sectortype      
    var w:_prwall;                                                                     //_prwall         
    var s:_prsector, ns:_prsector;                                                                //_prsector       
    var xref=0, yref=0;                                                             //int32_t         
    var ypancoef=0.0, dist=0.0;                                                         //float           
    var i=0;                                                                      //int32_t         
    var invalid=0;                                                                //uint32_t        
    var sectofwall = sectorofwall(wallnum);                                     //int32_t         
    ...
    if (w.wall.buffer == NULL) {
        w.wall.buffer = new Float32Array(4*5);//(GLfloat *)Bcalloc(4,  /*sizeof(GLfloat)*/ 4 * 5);  // XXX
        w.wall.vertcount = 4;
    }
    
    wallpicnum = wal.picnum;
    if (picanm[wallpicnum].sf & PICANM_ANIMTYPE_MASK) wallpicnum += animateoffs(wallpicnum, wallnum+16384);//DO_TILE_ANIM(wallpicnum, wallnum+16384);
    walloverpicnum = wal.overpicnum;
    if (walloverpicnum>=0)
        if (picanm[walloverpicnum].sf&PICANM_ANIMTYPE_MASK) walloverpicnum += animateoffs(walloverpicnum,  wallnum+16384);

    if (nwallnum >= 0 && nwallnum < numwalls)
    {
        nwallpicnum = wall[nwallnum].picnum;
        if (picanm[nwallpicnum].sf&PICANM_ANIMTYPE_MASK) nwallpicnum += animateoffs(nwallpicnum, wallnum+16384);//DO_TILE_ANIM(nwallpicnum, wallnum+16384);
    }
    else
        nwallpicnum = 0;
...
//        {
//            bglBindBufferARB(GL_ARRAY_BUFFER_ARB, w.stuffvbo);
//            bglVertexPointer(3, GL_FLOAT, 0, (const GLvoid*)(4 *  /*sizeof(GLfloat)*/ 4 * 5));
//        }
//        else
...
function polymer_updatesprite(/*int32_t*/ snum:number):void
{todoThrow();
    //var curpicnum=0, xsize=0, ysize=0, i=0, j=0;                  //int32_t         
    //var tilexoff=0, tileyoff=0, xoff=0, yoff=0, centeryoff=0;   //int32_t         
    //var tspr = tspriteptr[snum];                       //spritetype      
    //var xratio=0.0, yratio=0.0, ang=0.0, f=0.0;                         //float           
    //var spos = new Float32Array(3);                                        //float           
    //var inbuffer:Float32Array;                                      //const GLfloat   
    //var flipu=0.0, flipv=0.0;                                   //uint8_t         
    //var s:_prsprite;

    //var cs = tspr.cstat;                       //const uint32_t
    //var alignmask = (cs & SPR_ALIGN_MASK);     //const uint32_t
    //var flooraligned = (alignmask==SPR_FLOOR)?1:0;  //const uint8_t 
    ...
    //{
    //    prsprites[tspr.owner].init();// = (_prsprite *) Bcalloc(sizeof(_prsprite), 1);
    ...
    //    prsprites[tspr.owner].plane.buffer = (GLfloat *) Bcalloc(4,  /*sizeof(GLfloat)*/ 4 * 5);  // XXX
    ...
    //if (picanm[curpicnum].sf&PICANM_ANIMTYPE_MASK) curpicnum += animateoffs(curpicnum, tspr.owner+32768);
    ...
    //f = s.plane.material.diffusemodulation[3] * (1.0 - spriteext[tspr.owner].alpha);
    ...
    //{
    //case 0:
    //    ang =   /*(float)*/  ((viewangle) & 2047) / (2048.0 / 360.0);

    //    bglTranslatef(spos[0], spos[1], spos[2]);
    //    bglRotatef(-ang, 0.0, 1.0, 0.0);
    //    bglRotatef(-horizang, 1.0, 0.0, 0.0);
    //    bglTranslatef(  /*(float)*/  (-xoff),   /*(float)*/  (yoff), 0.0);
    //    bglScalef(  /*(float)*/  (xsize),   /*(float)*/  (ysize), 1.0);
    //    break;
    //case SPR_WALL:
    //    ang =   /*(float)*/  ((tspr.ang + 1024) & 2047) / (2048.0 / 360.0);

    //    bglTranslatef(spos[0], spos[1], spos[2]);
    //    bglRotatef(-ang, 0.0, 1.0, 0.0);
    //    bglTranslatef(  /*(float)*/  (-xoff),   /*(float)*/  (yoff-centeryoff), 0.0);
    //    bglScalef(  /*(float)*/  (xsize),   /*(float)*/  (ysize), 1.0);
    //    break;
    //case SPR_FLOOR:
    //    ang =   /*(float)*/  ((tspr.ang + 1024) & 2047) / (2048.0 / 360.0);
    ...
    //    bglTranslatef(spos[0], spos[1], spos[2]);
    //    bglRotatef(-ang, 0.0, 1.0, 0.0);
    //    bglTranslatef(  /*(float)*/  (-xoff), 1.0,   /*(float)*/  (yoff));
    //    bglScalef(  /*(float)*/  (xsize), 1.0,   /*(float)*/  (ysize));
    ...
    //Bmemcpy(s.plane.buffer, inbuffer,  /*sizeof(GLfloat)*/ 4 * 4 * 5);
    ...
    //            (s.plane.buffer[(i * 5) + 3] - 1.0) * -1.0;
    //        if (flipv)
    //            s.plane.buffer[(i * 5) + 4] =
    //            (s.plane.buffer[(i * 5) + 4] - 1.0) * -1.0;
    //    }
    //    while (++i < 4);
    //}
    ...
    //    bglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, 0, 4 *  /*sizeof(GLfloat)*/ 4 * 5, s.plane.buffer);
    //    bglBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);
    //}
...
//    bglTranslatef(pos[0], pos[1], pos[2]);
//    bglScalef(1000.0, 1000.0, 1000.0);
...
function polymer_initartsky():void
{
    var halfsqrt2 = 0.70710678;

    artskydata[0] = -1.0;          artskydata[1] = 0.0;           // 0
    artskydata[2] = -halfsqrt2;     artskydata[3] = halfsqrt2;      // 1
    artskydata[4] = 0.0;           artskydata[5] = 1.0;           // 2
    artskydata[6] = halfsqrt2;      artskydata[7] = halfsqrt2;      // 3
    artskydata[8] = 1.0;           artskydata[9] = 0.0;           // 4
    artskydata[10] = halfsqrt2;     artskydata[11] = -halfsqrt2;    // 5
    artskydata[12] = 0.0;          artskydata[13] = -1.0;         // 6
...
//        bglColor4f(glcolors[pskyoff[i]][0], glcolors[pskyoff[i]][1], glcolors[pskyoff[i]][2], 1.0);
...
//        bglBufferDataARB(GL_ARRAY_BUFFER_ARB, 4 *  /*sizeof(GLfloat)*/ 4 * 5 * 6, skyboxdata, modelvbousage);
...
//        if (pr_vbos > 0)
//        {
//            bglVertexPointer(3, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, (GLfloat*)(4 * 5 * i *  /*sizeof(GLfloat)*/ 4));
//            bglTexCoordPointer(2, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, (GLfloat*)(((4 * 5 * i) + 3) *  /*sizeof(GLfloat)*/ 4));
//        } else {
//            bglVertexPointer(3, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, &skyboxdata[4 * 5 * i]);
//            bglTexCoordPointer(2, GL_FLOAT, 5 *  /*sizeof(GLfloat)*/ 4, &skyboxdata[3 + (4 * 5 * i)]);
//        }
//        bglDrawArrays(GL_QUADS, 0, 4);
...
//        spos2[1] = -  /*(float)*/  (z - globalposz) / 16.0;
...
//        spos2[0] = spos2[1] = spos2[2] = 0.0;
//    }

//    ang =   /*(float)*/  ((tspr.ang+spriteext[tspr.owner].angoff) & 2047) / (2048.0 / 360.0);
//    ang -= 90.0;
//    if (((tspr.cstat>>4) & 3) == 2)
//        ang -= 90.0;
...
//        playerang = (globalang & 2047) / (2048.0 / 360.0) - 90.0;
//        radplayerang = (globalang & 2047) * 2.0 * PI / 2048.0;
//        cosminusradplayerang = cos(-radplayerang);
//        sinminusradplayerang = sin(-radplayerang);
//        hudzoom = 65536.0 / spriteext[tspr.owner].zoff;

//        bglTranslatef(spos[0], spos[1], spos[2]);
//        bglRotatef(horizang, -cosminusradplayerang, 0.0, sinminusradplayerang);
//        bglRotatef(spriteext[tspr.owner].roll / (2048.0 / 360.0), sinminusradplayerang, 0.0, cosminusradplayerang);
//        bglRotatef(-playerang, 0.0, 1.0, 0.0);
//        bglScalef(hudzoom, 1.0, 1.0);
//        bglRotatef(playerang, 0.0, 1.0, 0.0);
...
//    if (((tspr.cstat>>4) & 3) == 2)
//    {
//        bglTranslatef(0.0, 0.0, -  /*(float)*/  (tilesizy[tspr.picnum] * tspr.yrepeat) / 8.0);
//        bglRotatef(90.0, 0.0, 0.0, 1.0);
//    }
//    else
//        bglRotatef(-90.0, 1.0, 0.0, 0.0);

//    if ((tspr.cstat & 128) && (((tspr.cstat>>4) & 3) != 2))
//        bglTranslatef(0.0, 0.0, -  /*(float)*/  (tilesizy[tspr.picnum] * tspr.yrepeat) / 8.0);

//    // yoffset differs from zadd in that it does not follow cstat&8 y-flipping
//    bglTranslatef(0.0, 0.0, m.yoffset * 64 * scale * tspr.yrepeat);

//    if (tspr.cstat & 8)
//    {
//        bglTranslatef(0.0, 0.0,   /*(float)*/  (tilesizy[tspr.picnum] * tspr.yrepeat) / 4.0);
//        bglScalef(1.0, 1.0, -1.0);
//    }
...
//    bglTranslatef(0.0, 0.0, m.zadd * 64);
...
//        pitchang =   /*(float)*/  (spriteext[tspr.owner].pitch) / (2048.0 / 360.0);
//        rollang =   /*(float)*/  (spriteext[tspr.owner].roll) / (2048.0 / 360.0);

//        offsets[0] = -spriteext[tspr.owner].xoff / (scale * tspr.xrepeat);
//        offsets[1] = -spriteext[tspr.owner].yoff / (scale * tspr.xrepeat);
//        offsets[2] =   /*(float)*/  (spriteext[tspr.owner].zoff) / 16.0 / (scale * tspr.yrepeat);

//        bglTranslatef(-offsets[0], -offsets[1], -offsets[2]);

//        bglRotatef(pitchang, 0.0, 1.0, 0.0);
//        bglRotatef(rollang, -1.0, 0.0, 0.0);
...
//    {
//        double f = color[3] * (1.0 - spriteext[tspr.owner].alpha);
//        color[3] = (GLubyte)f;
//    }
...
//                lradius = prlights[i].range / 1000.0;

//                lpos[0] =   /*(float)*/  prlights[i].y;
//                lpos[1] = -  /*(float)*/  prlights[i].z / 16.0;
...
// MATERIALS
function  polymer_getscratchmaterial(material:_prmaterial):void
{
    ...
    material.frameprogress = 0.0;
    material.nextframedata = null;//(float*)-1;
    // PR_BIT_NORMAL_MAP
    material.normalmap = 0;
    material.normalbias[0] = material.normalbias[1] = 0.0;
    material.tbn = NULL;
    // PR_BIT_ART_MAP
    material.artmap = 0;
    ...
    material.diffusescale[0] = material.diffusescale[1] = 1.0;
    // PR_BIT_HIGHPALOOKUP_MAP
    material.highpalookupmap = 0;
    // PR_BIT_DIFFUSE_DETAIL_MAP
    material.detailmap = 0;
    material.detailscale[0] = material.detailscale[1] = 1.0;
    ...
    material.specmaterial[0] = 15.0;
    material.specmaterial[1] = 1.0;
    // PR_BIT_MIRROR_MAP
...
var polymer_getbuildmaterial_temp_buffer = new Uint8Array(640000);
function polymer_getbuildmaterial(material:_prmaterial, /*int16_t */tilenum:number, /*char */pal:number, /*int8_t */shade:number, /*int8_t */vis:number, /*int32_t */cmeth:number): void
{
    var pth:pthtyp;
    var /*int32_t*/usinghighpal = 0;
    ...
    if (pr_artmapping && (!pth || !pth.hicr) && tilenum != (MAXTILES - 4)) {
        ...
        //    var tilebuffer = waloff[tilenum];//char *tilebuffer = (char *)waloff[tilenum];
        //    var tempbuffer = polymer_getbuildmaterial_temp_buffer;//new Uint8Array(tilesizx[tilenum] * tilesizy[tilenum]);//char *tempbuffer = (char *)Bmalloc(tilesizx[tilenum] * tilesizy[tilenum]);
        //    if(tempbuffer.length < tilesizx[tilenum] * tilesizy[tilenum]) {todoThrow("temp buffer not large enough");}
        ...
        material.visibility = (uint8(vis+16) / 16.0);
...
function /* int32_t      */polymer_bindmaterial( material:_prmaterial, lights:Int16Array, /*int */matlightcount:number):number
{
    ...
    // --------- program compiling
    if (!prprograms[programbits] || !prprograms[programbits].handle)
...
function polymer_unbindmaterial(/*int32_t */programbits:number):void
{
...
function polymer_resetplanelights(plane:_prplane):void
{
    ...
    for (var i = 0; i < plane.lightcount; i++) {
        plane.lights[i] = -1;
    }
    plane.lightcount = 0;
}
...
function polymer_invalidateplanelights(plane:_prplane):void
{
    var/*int32_t         */i = plane.lightcount;
...
function polymer_invalidatesectorlights(/*int16_t*/ sectnum:number):void
{
    var/*int32_t         */i:number;
    var s = prsectors[sectnum];
...
//    radius =  /*(float)*/ (light.radius) / (2048.0 / 360.0);
//    ang =  /*(float)*/ (light.angle) / (2048.0 / 360.0);
//    horizang =  /*(float)*/ (-getangle(128, light.horiz-100)) / (2048.0 / 360.0);

//    bglMatrixMode(GL_PROJECTION);
//    bglPushMatrix();
//    bglLoadIdentity();
//    bgluPerspective(radius * 2, 1, 0.1, light.range / 1000.0);
...
//// RENDER TARGETS
function polymer_initrendertargets(/*int32_t */count:number):void
{todo();
//    int32_t         i;
