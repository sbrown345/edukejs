...
var hictinting: palette_t[] = newStructArray(palette_t, MAXPALOOKUPS);
...
//
function hicinit(): void
{
    var i: number,j: number;
    var hr: hicreplctyp, next: hicreplctyp;
    ...
    assert.run("hicinit char -1", hictinting[0].r == 255);
        ...
        for (i=MAXTILES-1; i>=0; i--)
        {
            for (hr=hicreplc[i]; hr;)
            {
                ...
                if (hr.skybox)
                {
                    for (j=5; j>=0; j--)
                        if (hr.skybox.face[j])
                            hr.skybox.face[j] = null;//Bfree(hr.skybox.face[j]);
                    hr.skybox = null;//Bfree(hr.skybox);
                }

                if (hr.filename)
                    hr.filename = null;//Bfree(hr.filename);
    ...
    memsetStruct(hicreplc,hicreplctyp,0,sizeof(hicreplc));
