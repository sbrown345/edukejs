...
var mapversion=7; // JBF 20040211: default mapversion to 7      //int32_t 
g_loadedMapVersion = -1;  // -1: none (e.g. started new)    //int32_t 
...
function clipmapinfo_init() : void
{
    ...
    if (sectq) { sectq = null; }
    if (sectoidx) { sectoidx= null; }
    if (tempictoidx) { tempictoidx = null; }

    for (i=0; i<MAXTILES; i++)
        pictoidx[i]=-1;

    if (loadsector) { loadsector = null;}
    if (loadwall) { loadwall = null; }
    if (loadwallinv) { loadwallinv = null; }
    if (loadsprite) { loadsprite = null; }
...
//static int32_t nrx1 = new Int32Array(8), nry1 = new Int32Array(8), nrx2 = new Int32Array(8), nry2 = new Int32Array(8); // JBF 20031206: Thanks Ken
...
class linetype{
  x1: number; y1: number; x2: number; y2: number;  //int32_t 
...
var clipit: linetype[] = newStructArray<linetype>(linetype, MAXCLIPNUM);
...
var apptitle = new Uint8Array(256);Bstrcpy(apptitle, "Build Engine");
...
function /*int32_t */rintersect(/*int32_t*/ x1:number, /*int32_t*/ y1:number, /*int32_t*/ z1:number,
                          /*int32_t*/ vx_:number, /*int32_t*/ vy_:number, /*int32_t*/ vz:number,
                          /*int32_t*/ x3:number, /*int32_t*/ y3:number, /*int32_t*/ x4:number, /*int32_t*/ y4:number,
                          /*int32_t*/ intx: R<number>, /*int32_t*/ inty: R<number>, /*int32_t*/ intz: R<number>): number
{
...
// raytrace (internal)
//
function /*int32_t */raytrace(/*int32_t */x3: number, /*int32_t */y3: number, /*int32_t */x4: R<number>, /*int32_t */y4: R<number>): number
{
    var /*int32_t */x1=0, y1=0, x2=0, y2=0, bot=0, topu=0, nintx=0, ninty=0, cnt=0, z=0, hitwall=0;
    var /*int32_t */x21=0, y21=0, x43=0, y43=0;
...
//# endif
    todo("init state_compress, state_decompress");
//todo state_compress = (qlz_state_compress *) Bcalloc(sizeof(qlz_state_compress) + sizeof(qlz_state_decompress), 1);
//todo state_decompress = (qlz_state_decompress *)((int8_t *)(state_compress) + sizeof(qlz_state_compress));
//#endif
...
//       <= -4: map-text error
function loadboard(filename: string, /*char*/ flags: number, dapos: vec3_t , /*int16_t*/ daang: R<number>, /*int16_t * */dacursectnum: R<number>): number
{
    var fil: number, i: number;
    var numsprites: number;
    var myflags = flags&(~3);
...
function loadpics(filename: string, askedsize: number): number
{
    ...
    memsetStruct(picanm, picanm_t, 0, sizeof(picanm));
    ...
    for (tilefilei=0; tilefilei<MAXTILEFILES; tilefilei++)
    {
        ...
        if ((fil = kopen4load(artfilename.toString(),0)) != -1)
        {
            ...
            artversion = kread32(fil); artversion = B_LITTLE32(artversion);
            if (artversion != 1)
            {
            ...
            numtiles_dummy = kread32(fil);
            localtilestart = kread32(fil); localtilestart = B_LITTLE32(localtilestart);
            localtileend = kread32(fil);   localtileend   = B_LITTLE32(localtileend);

            if (uint32(localtilestart) >= MAXTILES || uint32(localtileend) >= MAXTILES)
            {
...
// clipinsidebox
//
function /*int32_t */clipinsidebox(/*int32_t */x:number, /*int32_t */y:number, /*int16_t */wallnum:number, /*int32_t */walldist:number):number
{
    var wal:walltype;
    var /*int32_t */x1:number, y1:number, x2:number, y2:number;
...
function /*int32_t */getangle(/*int32_t */xvect:number, /*int32_t */yvect:number):number
{
    ...
    if (yvect == 0) return ((xvect<0?1:0)<<10);
    if (xvect == yvect) return 256+((xvect<0?1:0)<<10);
    if (xvect == -yvect) return 768+((xvect>0?1:0)<<10);
...
// ksqrt
//
function /*int32_t */ ksqrt(/*uint32_t*/ num:number):number
{
    return nsqrtasm(num);
}
...
// krecip
//
function/*int32_t */krecip(/*int32_t*/ num:number):number
{todoThrow();return -99999999999999;
...
// cansee
//
function /*int32_t */cansee(/*int32_t*/ x1:number, /*int32_t*/ y1:number, /*int32_t */z1:number, /*int16_t */sect1:number, /*int32_t */x2:number, /*int32_t */y2:number, /*int32_t */z2:number, /*int16_t */sect2:number): number
{
    var /*int32_t */dacnt:number, danum:number;
    var /*const int32_t */x21 = x2-x1, y21 = y2-y1, z21 = z2-z1;

    var sectbitmap = new Uint8Array(MAXSECTORS>>3);
//#ifdef YAX_ENABLE
...
//#endif
    Bmemset(new P(sectbitmap), 0, (numsectors+7)>>3);
//#ifdef YAX_ENABLE
restart_grand:
    for(;;) {
...
//#endif
    ...
    {
        var  /*const int32_t */dasectnum = clipsectorlist[dacnt];
        var /*const sectortype *const */sec = sector[dasectnum];
        var wal: walltype, walIdx: number;
        var /*int32_t */cnt:number;
//#ifdef YAX_ENABLE
        var cfz1 = new Int32Array(2), cfz2 = new Int32Array(2);  // both wrt dasectnum
        var bn = new Int16Array(2);

        var $bn0 = new R(bn[0]);
        var $bn1 = new R(bn[1]);
        yax_getbunches(dasectnum, $bn0, $bn1);
        bn[0] = $bn0.$;
        bn[1] = $bn1.$;
        var $cfz10 = new R(cfz1[0]);
        var $cfz11 = new R(cfz1[1]);
        getzsofslope(dasectnum, x1,y1, $cfz10, $cfz11);
        cfz1[0] = $cfz10.$;
        cfz1[1] = $cfz11.$;
        
        var $cfz20 = new R(cfz2[0]);
        var $cfz21 = new R(cfz2[1]);
        getzsofslope(dasectnum, x2,y2, $cfz20, $cfz21);
        cfz2[0] = $cfz20.$;
        cfz2[1] = $cfz21.$;
//#endif
        add_nextsector:
        for (cnt=sec.wallnum,walIdx = sec.wallptr, wal=wall[walIdx]; cnt>0; cnt--,wal = wall[++walIdx])
        {
            var wal2 = wall[wal.point2];
            var/*const int32_t */x31 = wal.x-x1, x34 = wal.x-wal2.x;
...
//#ifdef YAX_ENABLE
                if (t >= bot)
                {
                    var/*int32_t */cf:number, frac:number, ns:number;
                    for (cf=0; cf<2; cf++)
                    {
...
//#endif
            ...
            var $cfz0 = new R(cfz[0]);
            var $cfz1 = new R(cfz[1]);
            getzsofslope(dasectnum, x,y, $cfz0,$cfz1);
            cfz[0] = $cfz0.$;
            cfz[1] = $cfz1.$;
            ...
            {
//#ifdef YAX_ENABLE
                var /*int32_t */cf:number, frac:number;
...
//#endif
            var $cfz0 = new R(cfz[0]);
            var $cfz1 = new R(cfz[1]);
            getzsofslope(nexts, x,y, $cfz0,$cfz1);
            cfz[0] = $cfz0.$;
            cfz[1] = $cfz1.$;
            if (z <= cfz[0] || z >= cfz[1])
                return 0;
...
function /*int32_t */hitscan_trysector(/*const vec3_t **/sv: IVec3, sec:sectortype, hit:hitdata_t,
                                 ...
                                 /*uint16_t */stat:number, /*int16_t */heinum:number, /*int32_t */z:number, /*int32_t */how:number, /*const intptr_t **/tmp:Int32Array):number
{
    ...
    if (stat&2)
    {
        ...
        var wal2 = wall[wal.point2];
        var/*int32_t */j:number, dax=wal2.x-wal.x, day=wal2.y-wal.y;
...
// x1, y1: in/out
// rest x/y: out
function get_floorspr_points(spr:  spritetype, /*int32_t*/ px:number, /*int32_t*/ py:number,
                               /*int32_t*/ x1:R<number>, /*int32_t*/ x2:R<number>, /*int32_t*/ x3:R<number>, /*int32_t*/ x4:R<number>,
                               /*int32_t*/ y1:R<number>, /*int32_t*/ y2:R<number>, /*int32_t*/ y3:R<number>, /*int32_t*/ y4:R<number>): void
{
    var /*const int32_t */tilenum = spr.picnum;
    // &2047 in sinang:
    // DNE 1.3D lights camera action (1st level), spr.ang==2306
    // (probably from CON)
    var /*const int32_t */ cosang = sintable[(spr.ang+512)&2047];
    var /*const int32_t */ sinang = sintable[spr.ang&2047];

    var /*const int32_t*/ xspan=tilesizx[tilenum], xrepeat=spr.xrepeat;
    var /*const int32_t*/ yspan=tilesizy[tilenum], yrepeat=spr.yrepeat;

    var /*int32_t*/ xoff = picanm[tilenum].xofs + spr.xoffset;
    var /*int32_t*/ yoff = picanm[tilenum].yofs + spr.yoffset;
    var /*int32_t*/ k: number, l: number, dax: number, day: number;
...
function /*int32_t */try_facespr_intersect(spr:spritetype, refpos:IVec3,
                                     /*int32_t */vx:number, /*int32_t */vy:number, /*int32_t */vz:number,
...
function /*int32_t */clipmove(pos: IVec3, /*int16_t **/sectnum: R<number>,
                 ...
                 /*int32_t*/ walldist:number, /*int32_t*/ ceildist:number, /*int32_t*/ flordist:number, /*uint32_t*/ cliptype:number): number
{
    var sec2: sectortype;
    var /*int32_t */i:number, j:number, k:number, tempint1:number, tempint2:number;
    var /*int32_t*/ x1:number, y1:number, x2:number, y2:number;
    ...
    var /*const int32_t*/ dawalclipmask = (cliptype&65535);        //CLIPMASK0 = 0x00010001
    var /*const int32_t*/ dasprclipmask = (cliptype>>16);          //CLIPMASK1 = 0x01000040

    var /*const int32_t */oxvect=xvect, oyvect=yvect;

    var /*int32_t*/ goalx = pos.x + (xvect>>14);
    var /*int32_t*/ goaly = pos.y + (yvect>>14);
    var /*const int32_t*/ cx = (pos.x+goalx)>>1;
    var /*const int32_t*/ cy = (pos.y+goaly)>>1;
...
//initprintf("init sprite %d\n", clipspritecnt);
            if (!curspr)
            {
                ...
                Bmemcpy(new P(origclipsectorlist), new P(clipsectorlist), clipsectnum*sizeof(clipsectorlist[0]));
...
//    initprintf("sprite %d/%d: sect %d/%d (%d)\n", clipspritecnt,clipspritenum, clipsectcnt,clipsectnum,dasect);
        ...
        {
            var /*int32_t */clipyou = 0, dx: number, dy: number;
            var wal2 = wall[wal.point2];
...
//#endif
            ...
            {
                var /*int32_t */daz: number, daz2: number;
...
//#ifdef HAVE_CLIPSHAPE_FEATURE
    if (curspr)
    {
        ...
        Bmemcpy(new P(clipsectorlist), new P(origclipsectorlist), clipsectnum*sizeof(clipsectorlist[0]));
    }
//#endif
    ...
    do
    {
        var /*int32_t */intx=goalx, inty=goaly;
...
// pushmove
//
function /*int32_t */pushmove(/*vec3_t **/vect:IVec3, /*int16_t **/sectnum: R<number>,
                 /*int32_t */walldist:number, /*int32_t */ceildist:number, /*int32_t */flordist:number, /*uint32_t */cliptype:number):number
{
    var/*int32_t */i=0, j=0, k=0, t=0, dx=0, dy=0, dax=0, day=0, daz=0;
    var /*int32_t */dir=0, bad=0, bad2=0;
...
//    const int32_t dasprclipmask = (cliptype>>16);
    ...
    do
    {
        var /*int32_t */clipsectcnt:number;
        ...
        {
            var wal:walltype, walIdx:number ;
            var /*const */sec:sectortype ;
            var/*int32_t */startwall:number, endwall:number;
//#if 0
//            // Push FACE sprites
         ...
         //for (i=startwall,wal=&wall[startwall];      i!=endwall; i+=dir,wal+=dir)
            for(i=startwall,wal=wall[walIdx=startwall];i!=endwall; i+=dir,walIdx+=dir,wal=wall[walIdx])
                if (clipinsidebox(vect.x,vect.y,i,walldist-4) == 1)
                {
                    ...
                    if (j == 0)
                    {
                        var sec2 = sector[wal.nextsector];
                        var /*int32_t */daz2:number;
 ...
 * inlined code. */
function /*int32_t */inside_p(/*int32_t */x: number, /*int32_t */y: number, /*int16_t */sectnum: number): number
{
    return (sectnum>=0 && inside(x, y, sectnum) == 1) ? 1 : 0;
}
